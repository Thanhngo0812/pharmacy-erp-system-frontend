...
export default function AdminSidebar({
      ...
      if (tab.children) {
        return {
          ...
          children: tab.children.filter((child) => {
            if (!child.require_role || child.require_role.length === 0)
              return true;
            if (!user.roles) return false;
  ...
  return (
    <>
      ...
      {showSidebarTablet && (
        <div className="sidebar-overlay-tablet" onClick={toggleSidebar}></div>
      )}
      <div
        className={`sidebar-container ${openSidebar ? "" : "close"} ${showSidebarMobile ? "show-mobile" : ""} ${showSidebarTablet ? "show-tablet" : ""}`}
      >
        <FaTimes className="icon-close-mobile" onClick={toggleSidebar} />
            ...
            <div key={item.path} className="menu-item-wrapper">
              {item.children && item.children.length > 0 ? (
                <>
                  ...
                  >
                    ...
                    >
                      <span>{item.name}</span>
                      <span
                        className={`dropdown-icon ${expandedMenus[item.path] ? "expanded" : ""}`}
                      >
                        <FaChevronRight />
                      </span>
                    </div>
                  ...
                  <div
                    className={`sub-menu-container ${expandedMenus[item.path] && openSidebar ? "expanded" : ""}`}
                  >
                    <div className="sub-menu-list">
                      {item.children.map((child) => (
                        <NavLink
                          ...
                          key={child.path}
                          onClick={() => {
                            if (showSidebarMobile || showSidebarTablet)
                              toggleSidebar();
                          }}
                          className={({ isActive }) =>
                            isActive
                              ? "menu-items sub-menu-items active"
                ...
                <NavLink
                  ...
                  onClick={() => {
                    if (showSidebarMobile || showSidebarTablet) toggleSidebar();
                  }}
                  className={({ isActive }) =>
